...
class LSOprimizer:
    ...
    def score(self,nodes,labels,cluster_centers):
        ...
        """ define a function called metric_fun to include different parameters """
        if self.metric == "dtw":
                    def metric_fun(x, y):
        ...
        #def centers_init(X,n_cluster=self.k, cdist_metric=metric_fun):
            #return
        ...
        def update_centroid(self,GE):
            ...
            for k in range(self.k):
                if self.metric == "dtw":
                    ...
                    cluster_centers[k] = softdtw_barycenter(X=GE[self.labels == k,:,nodes],
        ...
        def compute_inertia(distances,labels,squared=True):
            ...
            if squared:
                ...
                return np.sum(distances[np.arange(n_ts), labels]) 
